...
[DACON: 월간 데이콘 음향 데이터 COVID-19 검출 AI 경진대회](https://dacon.io/competitions/official/235910/overview/description)에 참여하여 600명 이상의 참가자 중 **최종 22위 (상위 10% 이내)**를 달성한 프로젝트입니다.

---

## 1. 프로젝트 개요

COVID-19 검출을 위한 표준 방법인 RT-PCR 검사는 비용이 많이 들고 시간이 오래 걸린다는 한계가 있습니다. 특히, 바이러스 재유행으로 인해 검사량이 급증할 경우 의료 체계에 큰 부담이 될 수 있습니다.
...
* **주요 기술:** `Python`, `Librosa`, `Pandas`, `Scikit-learn`, `MLPClassifier`, `GridSearchCV`
...
* **모델 최적화 보조:** `RandomizedSearchCV`를 활용한 `MLPClassifier` 하이퍼파라미터 튜닝 
...
* **최종 리더보드 순위:** **22위** (상위 10% 이내) 
***최종 리더보드 점수 (Public Score):** **0.5936** 
